...
namespace Tot
{
    public class SourceQueryReader
    {
        ...
        public void Refresh()
        {
            ...
            {
                Console.WriteLine($"Endpoint={_endpoint.Address.ToString()}:{_endpoint.Port} {ex.SocketErrorCode}.");
                Online = false;
            }
            catch (Exception ex)
            {
                Console.WriteLine("Could not process Source Query");
        ...
        private void ReadResponse(MemoryStream ms, BinaryReader br)
        {
            ...
            Online = true;
            Console.WriteLine($"Endpoint={_endpoint.Address}:{_endpoint.Port} Name={Name}, Online={Online}, Players={Players}/{MaxPlayers}");
        }
